...
export default function App() {
  const { messages, isLoading, uploadedFiles, sendMessage, uploadPDF, deleteFile, clearFiles, regenerateMessage, deleteError, clearDeleteError } = useChat()
  ...
  return (
    <div className="flex h-screen bg-[var(--color-bg-main)] text-[var(--color-text-primary)] font-sans selection:bg-[var(--color-accent-blue)]/20 overflow-hidden">
      ...
      >
        ...
        {deleteError && (
          <div className="mx-4 mt-3 flex items-center justify-between gap-2 rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2 text-xs text-red-500">
            <span>{deleteError}</span>
            <button onClick={clearDeleteError} className="shrink-0 hover:opacity-70 transition-opacity">✕</button>
          </div>
        )}
